			...
			if success {
				...
				if sm.CommitIndex < msg.CommitIndex {
					...
					if sm.CommitIndex != -1 {
						...
						sm.actionCh <- Commit{sm.CommitIndex, sm.Log[sm.CommitIndex].Data, nil, false}
					} else {
						//fmt.Println("aaaaaaa2")
						sm.actionCh <- Commit{sm.CommitIndex, nil, nil, false}
					}
				}
			...
			if !msg.Success {
				...
				if sm.CommitIndex < n && sm.getLogTerm(n) == sm.CurrentTerm {
					...
					if sm.CommitIndex != -1 {
						...
						sm.actionCh <- Commit{sm.CommitIndex, sm.Log[sm.CommitIndex].Data, nil, true}
					} else {
						//fmt.Println("aaaaaaa4")
						sm.actionCh <- Commit{sm.CommitIndex, nil, nil, true}
					}
				}
			}
		}
