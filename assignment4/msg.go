...
type Msg struct {
	...
	Contents []byte
	NumBytes string
	Exptime  string // expiry time in seconds
	Version  string
	Id       int64
	Exp      string
}
...
func GetMsg(reader *bufio.Reader) (msg *Msg, msgerr error, fatalerr error) {
	...
	if fatalerr == nil {
		if msg.Kind2 == 'w' /*write*/ || msg.Kind2 == 'c' /*cas*/ || msg.Kind2 == 'C' /*CONTENTS*/ {
			numbytes, _ := strconv.Atoi(msg.NumBytes)
			msg.Contents, fatalerr = parseSecond(reader, numbytes)
		}
	}
...
func parseFirst(reader *bufio.Reader, buf []byte) (msg *Msg, msgerr error, fatalerr error) {
	...
	if fatalerr == nil {
		...
		exp := "2018-02-01 03:04:05 +0530 IST"

		if exptime != 0 {
			exp = time.Now().Add(time.Duration(exptime) * time.Second).String()
		}
		...
		return &Msg{Kind2: kind, Kind: kind2, Filename: filename, NumBytes: strconv.Itoa(numbytes), Exp: strconv.Itoa(exptime), Exptime: exp, Version: strconv.Itoa(version)}, msgerr, nil
	} else {
